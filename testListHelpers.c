...
// Fonction pour convertir la valeur d'un noeud
int node_to_string(s_node * node, char *buffer, int buffer_size) {
	int * value;
	int str_length;

	value=(int *)list_get_data(node);
	str_length=snprintf(buffer, buffer_size, "%d", *value);
	return (str_length >= buffer_size);  // voir man snprintf (return value)
}
